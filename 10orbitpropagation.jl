...
function get_A_prime_qns_augmented_koenig_selectable(ac_val::Float64, ec_val::Float64, ic_val::Float64, omegac_val::Float64, include_j2::Bool, include_drag_effects::Bool, drag_model_type::DragModelTypeForSTM, delta_B_val_param::Float64=0.0)::SMatrix{7,7,Float64}
    n_c=sqrt(mu_earth/ac_val^3); A_p=@MMatrix zeros(Float64,7,7)
    A_p[2,1]=-1.5*n_c
    if include_j2
        ...
        A_p[2,1]+=-0.5*kappa_J2*E_f*P_g; A_p[2,3]+=kappa_J2*F_f*G_f*ex_c; A_p[2,4]+=kappa_J2*F_f*G_f*ey_c; A_p[2,5]+=-kappa_J2*F_f*S_g
        A_p[3,4]+=-kappa_J2*Q_g; A_p[3,5]+=kappa_J2*Q_g*G_f*ey_c
        A_p[4,3]+=kappa_J2*Q_g; A_p[4,5]+=-kappa_J2*Q_g*G_f*ex_c
        A_p[6,1]+=0.5*kappa_J2*S_g; A_p[6,3]+=-kappa_J2*G_f*S_g*ex_c; A_p[6,4]+=-kappa_J2*G_f*S_g*ey_c; A_p[6,5]+=kappa_J2*T_g
    end
    if include_drag_effects
        if drag_model_type==DENSITY_MODEL_FREE
            A_p[1,7]=1.0; A_p[3,7]+=(1.0-ec_val)
        elseif drag_model_type==DENSITY_MODEL_SPECIFIC; println("警告: DENSITY_MODEL_SPECIFIC のプラント行列は未実装です。") end
    end
    return SMatrix(A_p)
end

function get_STM_prime_qns_augmented_koenig_model_selectable(A_prime_full_matrix::SMatrix{7,7,Float64}, t_prop::Float64, ec_val_for_drag_effect::Float64, include_j2::Bool, include_drag_effects::Bool, drag_model_type::DragModelTypeForSTM)::SMatrix{7,7,Float64}
    if drag_model_type==DENSITY_MODEL_FREE && include_drag_effects
        A_drag_only_p_m=@MMatrix zeros(Float64,7,7); A_drag_only_p_m[1,7]=1.0; A_drag_only_p_m[3,7]=(1.0-ec_val_for_drag_effect); A_drag_only_prime_s=SMatrix(A_drag_only_p_m)
        A_kj2_p_m=MMatrix{7,7,Float64}(A_prime_full_matrix); A_kj2_p_m[1,7]=0.0; A_kj2_p_m[3,7]=A_prime_full_matrix[3,7]-(1.0-ec_val_for_drag_effect)
        A_kep_J2_prime_s=SMatrix(A_kj2_p_m)
        Phi_drag_prime=SMatrix{7,7,Float64}(I)+A_drag_only_prime_s*t_prop
        Integral_Phi_drag_prime=SMatrix{7,7,Float64}(I)*t_prop+A_drag_only_prime_s*(t_prop^2/2.0)
        return Phi_drag_prime+A_kep_J2_prime_s*Integral_Phi_drag_prime
    elseif drag_model_type==DENSITY_MODEL_SPECIFIC && include_drag_effects; println("警告: DENSITY_MODEL_SPECIFIC のSTM計算は未実装です。"); return SMatrix{7,7,Float64}(I)+A_prime_full_matrix*t_prop
    else; return SMatrix{7,7,Float64}(I)+A_prime_full_matrix*t_prop; end
end
...
function main_simulation(
    ...
    for angle_val in 0.0:angle_step_val:(360.0-angle_step_val)
        ...
        roe_aug_init_vec=SVector(roe_aug_init_vec_temp); da0_val=roe_aug_init_vec[1]
        
        tf_val = 0.0; valid_tf_case = false
        if drag_model_setting == DENSITY_MODEL_FREE && include_drag_active_stm
            # 密度モデルフリーの場合は、δa=0になる時間を計算
            da_dot_eff_val = roe_aug_init_vec[7]
            if abs(da_dot_eff_val) > 1e-15 && da0_val * da_dot_eff_val < 0.0
                tf_val = -da0_val / da_dot_eff_val
                if tf_val > 1e-3
                    valid_tf_case = true
                end
            end
        else 
            # 抗力なし、または密度モデル依存(未実装)の場合は、固定時間（10軌道周期）とする
            tf_val = 10.0 * 2.0 * pi * sqrt(oe_chief_eval.a^3 / mu_earth)
            valid_tf_case = true
        end

        push!(angles_plot_list,angle_val)
        if !valid_tf_case; push!(roe_data_log[:final_delta_a],NaN); push!(roe_data_log[:final_delta_lambda],NaN); push!(roe_data_log[:final_delta_ex],NaN); push!(roe_data_log[:final_delta_ey],NaN); push!(roe_data_log[:final_delta_ix],NaN); push!(roe_data_log[:final_delta_iy],NaN); push!(roe_data_log[:cost],Inf); push!(roe_data_log[:t_final],NaN); push!(roe_data_log[:final_rel_j2_r],NaN); push!(roe_data_log[:final_rel_j2_t],NaN); push!(roe_data_log[:final_rel_j2_n],NaN); push!(roe_data_log[:final_rel_j2_norm],NaN); continue; end
        ...
        A_prime=get_A_prime_qns_augmented_koenig_selectable(oe_chief_eval.a,oe_chief_eval.e,oe_chief_eval.i,omega_c_ti,include_j2_active,include_drag_active_stm,drag_model_setting,delta_B_initial_param)
        STM_prime=get_STM_prime_qns_augmented_koenig_model_selectable(A_prime,tf_val,oe_chief_eval.e,include_j2_active,include_drag_active_stm,drag_model_setting)
        ...
        cost_da_penalty=(drag_model_setting==DENSITY_MODEL_FREE&&include_drag_active_stm) ? (1000.0*roe_aug_final_vec[1]^2) : 0.0 # δa=0を目指す場合のみペナルティ
        ...
        aj2_chief_final_eci=calculate_j2_perturbation_eci(r_chief_eci_final_val,mu_earth,J2_coeff,R_E)
        aj2_deputy_final_eci=calculate_j2_perturbation_eci(r_deputy_eci_final_val,mu_earth,J2_coeff,R_E)
        aj2_relative_final_eci=aj2_deputy_final_eci-aj2_chief_final_eci
        aj2_relative_final_hill=eci_to_hill_relative_acceleration(aj2_relative_final_eci,r_chief_eci_final_val,v_chief_eci_final_val)
    ...
    println("ループ終了")
    println("\n--- 結果 (Pert: $perturbation_setting, DragModel: $drag_model_setting) ---")
